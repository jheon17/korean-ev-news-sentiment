...
- 완료: 1차 수집·전처리, 초기 모델 비교, 35건 라벨 재검토, 프롬프트 v2 작성, 평가 split 처리 및 실행 범위별 결과 파일 분리
...
dev 전체 실행은 기존 파일명을 유지하고, 그 외 split·limit 조합은 실행 범위를 붙인 별도 파일에 저장합니다.
...
1. 긍정·중립·부정 사례를 다양하게 포함하도록 새 데이터를 수집하고 평가셋 설계
2. 같은 사건의 유사 기사가 split 사이에 섞이지 않도록 분리 방식 개선
3. 새 평가셋에서 기존 모델 재평가
4. 재평가 후 프롬프트 v1과 v2 비교
5. 날짜 하드코딩과 실행 인자 정리
6. 장기적으로 월별 감성 지수 생성

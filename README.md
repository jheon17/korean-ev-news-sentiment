...
현재는 1차 실험이 끝난 상태입니다. 데이터 수집부터 전처리, 사람 정답 라벨 작성,
규칙 기반 모델·Qwen·OpenAI API 비교, 오분류 분석과 프롬프트 v2 작성까지 진행했습니다.

- 완료: 1~8단계, 9단계 오분류 분석 및 프롬프트 v2 작성
- 진행 예정: 프롬프트 v2 실행·비교, 최종 테스트 평가
- 미진행: 장기간 뉴스 수집과 월별 감성 지수 생성

현재 성능은 최종 성능이 아니라 **개발용 데이터 28건에 대한 중간 결과**입니다.
...
| 5 | 사람 정답 35건 작성, dev/final_test 분리 | [정답 라벨](docs/step5_human_labels.md) | 완료 |
| 6 | 키워드 규칙 기반 기준 모델 평가 | [기준 모델](docs/step6_baseline_model.md) | 완료 |
| 7 | 로컬 모델과 OpenAI API 분류 | [모델 선택](docs/step7_model_selection.md) | 완료 |
| 8 | Accuracy, Macro-F1, 클래스별 지표 비교 | [평가](docs/step8_evaluation.md) | 완료 |
| 9 | 오분류 유형화와 프롬프트 v2 작성 | [오류 분석](docs/step9_error_analysis.md) | 일부 완료 |
| 10 | 월별 감성 지수 산출 | AGENTS.md 10단계 | 미진행 |

전체 프로젝트 설명은 [PDF 보고서](reports/project_report_ev_news_sentiment.pdf)에서도 확인할 수 있습니다.

## 1차 실험 결과

평가 범위는 `dev` 28건입니다. Qwen의 Accuracy와 Macro-F1은 분류에 성공한 24건을
기준으로 계산됐으며, 별도로 4건의 출력 파싱 실패가 있었습니다.
...
- 현재 정답 데이터는 35건이며 모두 2026-08-03의 짧은 수집 구간에 속합니다.
- dev는 긍정 21건, 중립 3건, 부정 4건으로 클래스가 불균형합니다.
- final_test는 7건뿐이고 특정 검색어에 치우쳐 최종 성능을 대표하기 어렵습니다.
- 사람 라벨은 한 명이 작성했으므로 라벨 기준에 주관이 포함될 수 있습니다.
- OpenAI 비용은 코드에 둔 단가와 환율을 이용한 추정치이며 실제 청구액과 다를 수 있습니다.
- 현재 GPT 결과는 프롬프트 v1의 dev 점수이므로 서비스 성능으로 해석하면 안 됩니다.

## 다음 작업

1. 라벨 근거를 재검토하고 100~200건으로 데이터 확대
2. dev에서 프롬프트 v1과 v2 비교
3. 프롬프트 확정 후 별도의 최종 테스트 세트를 한 번만 평가
4. 날짜 하드코딩 제거와 실행 인자 정리
5. 여러 달의 뉴스를 수집한 뒤 월별 감성 지수 생성
